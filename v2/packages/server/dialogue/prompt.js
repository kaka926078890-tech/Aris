...
/** 不含场景操作说明；场景块见 SCENE_RULES_DEFAULT */
const BASE_CONVERSATION_RULES = `根据当前情境与用户情绪调整语气。自然对话优先：本回合上下文中已提供的身份、约束、近期对话与小结等可直接依据回答；不要为了用工具而用工具。仅当问题超出本回合上下文、需从更早对话或向量记忆中查找时再调用 get_record / search_memories。
本回合会在上下文中提供【禁止用语】与【用户约束摘要】或【用户约束】全文，须严格遵守；若同时出现摘要与全文，以全文为准。
