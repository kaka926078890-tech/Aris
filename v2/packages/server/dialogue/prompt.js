...
const DEFAULT_CONVERSATION_RULES = `根据当前情境与用户情绪调整语气。每轮【用户约束】已包含用户要求、纠错记录、用户喜好、禁止用语，回答时直接依据该块即可，无需为此调用 search_memories；仅当问题超出该块、需从更早对话或向量记忆中查找时才检索。需要查看项目内代码/文件或列出目录结构时，可先调用 get_dir_cache / get_read_file_cache 复用仍有效的目录与文件摘要，再对未读或已变更的内容按需 list_my_files/read_file。可用 record（type 为 self_note，payload 传 note）记录自我反思（不写敏感信息）。`;
 ...
 * @param {object} dto - 来自 contextBuilder.buildContextDTO：userIdentity, userConstraints, contextWindow, lastStateAndSubjectiveTime, relatedAssociations, recentSummary, emotionLine?, behavioralRules?
 */
