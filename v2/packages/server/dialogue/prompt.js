...
// 对话规则 = 每轮行为与工具使用（语气、记忆路径、重启、缓存、record）；行为规则 = rules.md 中用户/项目自定义条款（可为空）。
const DEFAULT_CONVERSATION_RULES = `根据当前情境与用户情绪调整语气。自然对话优先：本回合上下文中已提供的身份、约束、近期对话与小结等可直接依据回答；不要为了用工具而用工具。仅当问题超出本回合上下文、需从更早对话或向量记忆中查找时再调用 get_record / search_memories。每轮【用户约束】已包含用户要求、纠错记录、用户喜好、禁止用语，直接依据该块即可，无需为此单独检索。
...
// 【行为规则】= rules.md 用户/项目自定义条款；上文对话规则 = conversation_rules。【近期小结】= 本会话中更早对话的摘要。空块不传，只拼有内容的块。
const EMPTY = ['（无）', '（暂无）'];
function isEmpty(v) {
...
/**
 * BFF：仅根据上下文 DTO + 配置 组装一条 system prompt。不读 store，不执行工具。有内容的块才拼接，空块不传。
...
function buildSystemPrompt({
  ...
  const blocks = [];
  if (!isEmpty(userIdentity)) blocks.push('【用户身份】\n\n' + userIdentity);
  if (!isEmpty(userConstraints)) blocks.push('【用户约束】以下为要求、纠错、喜好、禁止用语的完整汇总，请每轮遵守。\n\n' + userConstraints);
  if (!isEmpty(lastStateAndSubjectiveTime)) blocks.push('【你上一次的状态与时间感】\n\n' + lastStateAndSubjectiveTime);
  if (!isEmpty(relatedAssociations)) blocks.push('【相关关联】\n\n' + relatedAssociations);
  if (!isEmpty(recentSummary)) blocks.push('【近期小结】（本会话中更早对话的摘要）\n\n' + recentSummary);
  if (!isEmpty(contextWindow)) blocks.push('【当前会话最近几轮】\n\n' + contextWindow);
  if (!isEmpty(behavioralRules)) blocks.push('【行为规则】\n\n' + behavioralRules);
  let system = PERSONA + '\n\n以下是你需要参考的上下文：\n\n' + blocks.join('\n\n');
