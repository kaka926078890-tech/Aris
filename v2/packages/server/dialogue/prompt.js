...
const DEFAULT_CONVERSATION_RULES = `根据当前情境与用户情绪调整语气。每轮【用户约束】已包含用户要求、纠错记录、用户喜好、禁止用语，回答时直接依据该块即可，无需为此调用 search_memories；仅当问题超出该块、需从更早对话或向量记忆中查找时才检索。可用 record（type 为 self_note，payload 传 note）记录自我反思（不写敏感信息）。`;
