...
- **v3 最小后端**（`v3/serve`，独立进程与数据目录）：环境变量以 `v3/serve/.env.example` 为准，说明见 `v3/serve/README.md`（含提示词预算、记忆检索、**会话 compaction**、检索时间衰减 `PROMPT_RETRIEVAL_TIME_DECAY_PER_DAY` 等）。提示词与记忆、工具层的**最终实现规格**见 [`v3/docs/prompt-memory-tool-final-spec.md`](../v3/docs/prompt-memory-tool-final-spec.md)。
